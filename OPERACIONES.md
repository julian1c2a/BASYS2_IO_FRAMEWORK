...
- **Lógica:** La memoria de salida es una copia byte a byte de la memoria de entrada.
- **Utilidad:** Verifica que la interfaz de memoria, el sistema de ventanas de salida y el handshake `START`/`READY` funcionan correctamente.
- **Implementación actual (`OP_IDENTIDAD.vhdl`):**
  - Arquitectura con **dos procesos síncronos** separados:
    - `P_CONTROL`: gestiona la FSM (`ST_IDLE`/`ST_RUN`), el índice `s_idx` y la señal `READY`.
    - `P_MEM_IF`: gestiona la escritura en la memoria de salida (`OUT_WE`, `OUT_WADDR`, `OUT_WDATA`).
  - La dirección de lectura de la memoria de entrada (`IN_RADDR`) se asigna concurrentemente desde `s_idx`.
  - Flujo: `IN_MEMORY` → `OP_IDENTITY` → `OUT_MEMORY` (TOP no interactúa directamente con la operación).
