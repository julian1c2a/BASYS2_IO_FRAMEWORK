...
El sistema utiliza dos módulos de memoria RAM separados (entidad `MEMORY`):

| Módulo              | Escritor                       | Lector                | Uso                        |
| ------------------- | ------------------------------ | --------------------- | -------------------------- |
| `IN_MEMORY_MODULE`  | TOP (estado `ST_IN`)           | `OP_IDENTITY`         | Buffer de entrada de datos |
| `OUT_MEMORY_MODULE` | `OP_IDENTITY` (estado `ST_OP`) | TOP (estado `ST_OUT`) | Buffer de resultados       |

Cada módulo `MEMORY` expone 1 puerto de escritura síncrona y 2 puertos de lectura asíncrona. Los buses están tipados con `ABUS_t` (dirección) y `DBUS_t` (dato), definidos en `GENERAL.MEMORY_TYPES`.

## Especificaciones Técnicas

- **Cálculo de constante:** Para `GEN_IO_CLK`, usar `MAX_COUNT = 12,500,000`.
- **Manejo de tipos:** Los buses de dirección y dato se declaran como `ABUS_t` y `DBUS_t` (ambos `UNSIGNED` de 8 bits). Para nuevas operaciones, reutilizar estos subtipos en todos los puertos de memoria.
- **Separación de responsabilidades en operaciones:** Cada módulo de operación debe implementar dos procesos síncronos: `P_CONTROL` (FSM + handshake) y `P_MEM_IF` (interfaz de escritura en memoria de salida).
